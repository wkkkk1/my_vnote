...
```
更新了一下系统
需要对code和code的应用市场插件进行安装

# 2025.6.30
继续跟进那本书，使用crosstool-NG构建工具链
 git clone https://github.com/crosstool-ng/crosstool-ng.git 

  编译qemu工具链的时候得下载东西。
  
```bash
git clone https://gitcode.com/gh_mirrors/cros/crosstool-NG.git
 1793  cd crosstool-NG
 1795  git checkout crosstool-ng-1.24.0
 1797  ./bootstrap
 1799  sudo pacman -S help2man
 1805  ./configure --prefix=/opt/crosstool-ng\n
 1806  make
 1807  make install
 1808  sudo make install
 1810  /opt/crosstool-ng/bin/ct-ng distclean\n
 1812  /opt/crosstool-ng/bin/ct-ng arm-unknown-linux-gnueabi\n
 1816  /opt/crosstool-ng/bin/ct-ng menuconfig
 1817  /opt/crosstool-ng/bin/ct-ng build\n
 1818  wget https://www.kernel.org/pub/linux/kernel/v4.x/linux-4.20.8.tar.xz
 1819  wget https://mirrors.edge.kernel.org/pub/linux/kernel/v4.x/linux-4.20.8.tar.xz
 1820  ls
...
2025.06.10
```bash
west build -p always -b rpi_pico2/rp2350a/m33  ./  -- -DCONFIG_USB_DEVICE_INITIALIZE_AT_BOOT=y

#或者
cmake -Bbuild -GNinja    -DBOARD=rpi_pico2/rp2350a/m33  ./
ninja  -C build 
cp build/zephyr/zephyr.uf2 /run/media/wkkkk/RP2350/
```

可以成功烧录到pico2,而且连接了相关的串口后，特别方便

接下来的问题是，烧录到那个东西，  解决了树莓派的烧录，特别好用。

而且那个破板子的烧录也能实现，就是接线不方便，回家一是windows得切换才能使用那个ide进行烧录，，二是来回拔线，很麻烦。
先这样吧，回家看能不能出工程，
